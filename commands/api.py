...
from ..core.io import parse_key_values, read_json_file
...
def add_api_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    stats.add_argument("--format", choices=["json", "text"], default="json", help="Render output as json or text")
    ...
    params_parser.add_argument("--format", choices=["json", "text"], default="json", help="Render output as json or text")
    ...
    call_parser.add_argument("--dry-run", action="store_true", help="Show the prepared request without executing it")
    call_parser.add_argument("--format", choices=["json", "text"], default="json", help="Render output as json or text")
...
def handle_api(args: argparse.Namespace) -> int:
    ...
    if args.api_command == "stats":
        emit_payload(registry.stats(), output=args.output, fmt=args.format)
        return 0
    ...
    if args.api_command == "show":
        emit_payload(registry.get(args.path).raw, output=args.output, fmt=args.format)
        return 0
    ...
    if args.api_command == "call":
        ...
        payload = client.describe_prepared(prepared) if args.dry_run else client.call(prepared, parse_atom=True)
        emit_payload(payload, output=args.output, fmt=args.format)
        return 0
